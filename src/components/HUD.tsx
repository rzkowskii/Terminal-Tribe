...
const HUD: React.FC = () => {
  const { setShowMap, setShowCodex, showSceneEffects, toggleSceneEffects, autoAdvance, setAutoAdvance, setShowSettings, featureFlags, setShowSkills, setShowProfile } = useUiStore();
  const { levels, currentLevel } = useLevelStore();
  const total = levels.length || 150;
  const shown = Math.min(Math.max(currentLevel, 1), total);
  return (
    <div className="flex items-center justify-between mb-4">
      <div className="text-terminal-text/80">Progress: {shown}/{total}</div>
      <div className="flex items-center gap-2">
        ...
        {/* Scene toggle moved to Settings: Experimental Scene */}
